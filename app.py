...
if __name__ == '__main__':
    ...
    while True:
        ...
        if len(new_open_courses) == 0:
            ...
            webbrowser.open('https://apps.es.vt.edu/StudentRegistrationSsb/ssb/classRegistration/classRegistration')
            server = smtplib.SMTP('smtp.gmail.com', 587)
            server.starttls()
            #Next, log in to the server
            server.login("bharathc", "neymar98")
            #Send the mail
            gmail_user = "bharathc@gmail.com"
            TO = 'pbc8ze@virginia.edu'
            SUBJECT = first_course.crn
            TEXT = "CRN is in subject line "
            BODY = '\r\n'.join(['To: %s' % TO,
            'From: %s' % gmail_user,
            'Subject: %s' % SUBJECT,
            '', TEXT])
            server.sendmail("bharathc@gmail.com", "jaic4@vt.edu", BODY)
